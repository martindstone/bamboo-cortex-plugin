const listKeyForUrl = (url: string) => {
...
export const fetchPaginated = async (url: string, params: FetchPaginatedParams = {urlParams: {}}): Promise<any[]> => {
  let pagesFetched = 0;
  let data: any[] = [];
  ...
  while (true) {
    ...
    const response = await fetch(`${reqUrl}?${new URLSearchParams(reqParams)}`);
